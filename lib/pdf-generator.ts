...
export const generateFichaPDF = (formData: FormData, fichaId?: string) => {
  ...
  doc.text('FICHA ODONTOLÓGICA', doc.internal.pageSize.width / 2, 20, { align: 'center' });

  doc.setFontSize(textFontSize);
  doc.setFont('helvetica', 'normal');
  doc.text(`ID Ficha: ${fichaId || '________________'}`, doc.internal.pageSize.width - margin, 20, { align: 'right' });
  doc.text(`Fecha de Emisión: ${new Date().toLocaleDateString('es-ES')}`, doc.internal.pageSize.width - margin, 25, { align: 'right' });

  // --- FUNCIÓN AUXILIAR PARA AÑADIR SECCIONES ---
  const addSection = (title: string, body: (string | number)[][], startY: number) => {
  ...
  // --- 1. FILIACIÓN ---
  const filiacionData = [
    ['Nombres y Apellidos', `${formData.filiacion.nombres} ${formData.filiacion.apellidos}`],
  ...
  for (const [key, value] of Object.entries(formData.antecedentes)) {
    const titulo = key.charAt(0).toUpperCase() + key.slice(1).replace(/_/g, ' ');
    
    // Aseguramos el tipo de 'value' para que TypeScript no lo infiera como 'unknown'
    const seccion = value as { no_refiere: boolean; [key: string]: boolean | string | { [key: string]: boolean } };
    ...
    if (seccion.no_refiere) {
      ...
      const detalles = Object.entries(seccion)
        ...
        .map(([subKey, subValue]) => {
          ...
          if (typeof subValue === 'string' && subValue) return `${subKey.replace(/_/g, ' ')}: ${subValue}`;
          ...
          if (typeof subValue === 'object' && subValue !== null) {
            const nestedDetails = Object.entries(subValue)
              ...
              .map(([k]) => k)
              .join(', ');
            if (nestedDetails) return `${subKey.replace(/_/g, ' ')}: ${nestedDetails}`;
          }
          return null;
        })
        .filter(Boolean)
  ...
  // --- 3. HÁBITOS ---
  const habitosData = [
    ['Tabaco', `${formData.habitos.tabaco} ${formData.habitos.tabaco_actual_detalle ? `(${formData.habitos.tabaco_actual_detalle})` : ''}`],
    ['Alcohol', `${formData.habitos.alcohol} ${formData.habitos.alcohol_frecuente_detalle ? `(${formData.habitos.alcohol_frecuente_detalle})` : ''}`],
    ['Drogas Recreacionales', formData.habitos.drogas_recreacionales ? `Sí (${formData.habitos.drogas_tipo})` : 'No'],
  ];
  ...
  if (formData.seguimiento.length > 0) {
    ...
    const seguimientoHead = [['Fecha', 'Procedimiento Realizado', 'Observaciones', 'Próxima Cita']];
