...
import type { FichaPDFPayload, PresupuestoPDFPayload, AntecedentesAnnotation, OdontogramaData, Seguimiento, PresupuestoItem, PagoItem, TableBody } from "@/lib/types/pdf";
...
export const generateFichaPDF = (payload: FichaPDFPayload): ArrayBuffer => {
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(255, 255, 255);
  doc.text(`Nº Historia: ${numeroHistoria}`, pageWidth / 2, 32, { align: "center" });
  ...
  const edad = filiacion.fecha_nacimiento ? calcularEdad(filiacion.fecha_nacimiento) : "";
  const sexo = filiacion.sexo === 'masculino' ? 'Masculino' : filiacion.sexo === 'femenino' ? 'Femenino' : "";
  
  const filiacionData = [
    [
      { content: "Nombres: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
      { content: filiacion.nombres && filiacion.apellidos ? `${filiacion.nombres} ${filiacion.apellidos}` : (filiacion.nombres || ""), colSpan: 2 },
      { content: "Edad: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
      edad || "",
      { content: "Sexo: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
      sexo || ""
    ],
    [
      { content: "Dirección: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
      { content: filiacion.direccion || "", colSpan: 2 },
      { content: "Est. civil: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
      filiacion.estado_civil || "",
      { content: "", colSpan: 1 }
    ],
    [
      { content: "Ocupación: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
      { content: filiacion.ocupacion || "", colSpan: 2 },
      { content: "Teléfono: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
      { content: filiacion.telefono || "", colSpan: 2 }
    ],
    [
      { content: "País/Dpto.: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
      { content: filiacion.lugar_procedencia || "", colSpan: 2 },
      { content: "F. Nac.: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
      { content: filiacion.fecha_nacimiento || "", colSpan: 2 }
    ],
    [
      { content: "Recomendado por: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
      { content: filiacion.recomendado_por || "", colSpan: 2 },
      { content: "Email: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
      { content: filiacion.email || "", colSpan: 2 }
    ],
    [
      { content: "Fecha: ", styles: { fontStyle: "bold", fontSize: 8, textColor: COLORS.primary } },
  ...
  Object.entries(antecedentes).forEach(([categoria, datos]: [string, unknown]) => {
    antecedentesMap.set(normalizeKey(categoria), datos as Record<string, unknown>);
  });
  ...
  ordenCategorias.forEach((catKey) => {
    ...
    if (datos.no_refiere) {
      antecedentesBody.push([
        { content: titulo, styles: { fontStyle: "bold" as const, textColor: COLORS.primary } },
        { content: "No refiere", styles: { textColor: COLORS.textLight, italic: true } }
      ] as unknown as Record<string, unknown>);
      return;
    }

    const detalles: string[] = [];
    const questions = (datos.questions && typeof datos.questions === "object") ? datos.questions : datos;

    Object.entries(questions).forEach(([campo, valor]) => {
      if (campo === "no_refiere" || campo === "annotations" || campo === "annotationsEnabled") return;
      ...
      if (typeof valor === "boolean") {
        ...
        const texto = (valueObj.texto as string) || (valueObj.detalle as string) || "";
    ...
    const anotaciones = Array.isArray(dataObj.annotations) ? (dataObj.annotations as AntecedentesAnnotation[]) : [];
    const anotacionesLimpias = anotEnabled && anotaciones.length > 0 
      ? anotaciones
    ...
    if (detalles.length > 0 || anotacionesLimpias.length > 0) {
      ...
      const anotacionesTotales = anotacionesLimpias.length > 0 
  ...
  if (antecedentesBody.length > 0) {
    autoTable(doc, {
      ...
      headStyles: { fillColor: COLORS.bgAlt, textColor: COLORS.primary, fontStyle: "bold", halign: "center" },
      columnStyles: {
  ...
  const odontograma = (payload.odontograma as OdontogramaData | undefined) || {};
  if ((odontograma as OdontogramaData).existe && (odontograma as OdontogramaData).version) {
    const odontoImg: string | undefined = ((odontograma as OdontogramaData).imagen_base64) || ((odontograma as unknown as Record<string, unknown>).imagen_url as string);
    
    if (odontoImg) {
      try {
        const imgFormat = odontoImg.startsWith("data:image/png") ? "PNG" : "JPEG";
    ...
    const odontogramaDetalles = [
      [
        { content: "Versión", styles: { fontStyle: "bold", textColor: COLORS.primary } },
        `v${odontoData.version}`,
        { content: "Fecha", styles: { fontStyle: "bold", textColor: COLORS.primary } },
        odontoData.fecha_registro || ""
      ],
      [
        { content: "Observaciones", styles: { fontStyle: "bold", textColor: COLORS.primary } },
        { content: odontoData.observaciones || "Sin observaciones", colSpan: 3, styles: { textColor: COLORS.text } }
      ]
    ];
  ...
  if (seguimientos.length > 0) {
    const seguimientoHead = [["Fecha", "Tratamientos Realizados", "Observaciones"]];
    const seguimientoBody = seguimientos.map((seg: Seguimiento) => [
      seg.fecha || "",
      seg.tratamientos && seg.tratamientos.length > 0 ? seg.tratamientos.join("\n") : "-----",
      ""
    ]);
...
// ==================== PRESUPUESTO ====================
export const generatePresupuestoPDF = (payload: PresupuestoPDFPayload): ArrayBuffer => {
  const doc: jsPDFWithAutoTable = new jsPDF();
  let finalY = 40;
  ...
  const fechaPresupuesto = payload.fecha_presupuesto || new Date().toLocaleDateString('es-ES');
  const numeroHistoria = payload.numero_historia || "";
  const correlativo = payload.correlativo ? String(payload.correlativo).padStart(3, "0") : "---";

  doc.setFont("helvetica", "bold");
  doc.setFontSize(20);
  doc.setTextColor(255, 255, 255);
  doc.text(`Presupuesto #${correlativo}`, pageWidth / 2, 18, { align: "center" });

  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);
  doc.text(`Fecha: ${fechaPresupuesto}`, pageWidth / 2, 28, { align: "center" });
  doc.text(`Nº Historia: ${numeroHistoria}`, pageWidth / 2, 36, { align: "center" });
  ...
  if (items.length > 0) {
    const itemsHead = [["#", "Tratamiento", "Descripción", "Cantidad", "P. Unit.", "Sub total"]];
    ...
    itemsBody.push([
      { content: "PRESUPUESTO TOTAL", colSpan: 5, styles: { fontStyle: "bold" as const, halign: "right" as const, textColor: [255, 255, 255], fillColor: COLORS.success } } as unknown as string,
      { content: `${monedaSimbolo} ${totalPresupuesto.toFixed(2)}`, styles: { fontStyle: "bold" as const, textColor: [255, 255, 255], fillColor: COLORS.success } } as unknown as string
    ]);
...
function drawSectionHeader(doc: jsPDFWithAutoTable, x: number, y: number, title: string) {
