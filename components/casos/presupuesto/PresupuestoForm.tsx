...
export function PresupuestoForm({ casoId, pacienteId, numeroHistoria, presupuesto }: PresupuestoFormProps) {
    ...
    const loadData = async () => {
      try {
        ...
        if (presupuesto?.moneda_id && monedasData.data) {
          const found = (monedasData.data as Array<{id: string; nombre: string; codigo: string}>).find((m) => m.id === presupuesto.moneda_id)
      ...
      try {
        ...
        if (user?.email) {
          ...
          if (personalMatch && personalMatch.length > 0) {
            setCurrentPersonal({ id: personalMatch[0].id, nombre_completo: personalMatch[0].nombre_completo, rol: personalMatch[0].rol })
    ...
    const convertPrices = async (newMonedaId: string) => {
      ...
      try {
        ...
        if (preciosError) {
          ...
          toast.error('No se pudieron obtener precios para la moneda seleccionada.')
          return
        }
        ...
        setItems((prev) => prev.map((it) => {
          ...
          if (price !== undefined) {
            const rounded = Math.round((Number(price) + Number.EPSILON) * 100) / 100
            return {
              ...it,
              costo: rounded,
              moneda_id: newMonedaId,
              total: rounded * it.cantidad - (rounded * it.cantidad * it.descuento_porcentaje) / 100,
            }
          }
  ...
  const handleAddProcedimiento = (procedimientos: Array<{ id: string; nombre: string; precioDefault: number; cantidad: number }>) => {
    ...
    toast.success(addedCount > 1 ? `${addedCount} procedimientos insertados correctamente.` : 'Procedimiento insertado correctamente.', { style: { backgroundColor: '#008000', color: 'white' } })
  }
  ...
  const handleSave = async () => {
    if (!asunto.trim()) {
      toast.error('El asunto es requerido.', { style: { backgroundColor: '#FF0000', color: 'white' } })
      return
    }

    // medicId debe estar presente (se autocompleta con el usuario actual)
    if (!medicId) {
      toast.error('No se pudo determinar el usuario actual. Intenta refrescar la página.', { style: { backgroundColor: '#FF0000', color: 'white' } })
      return
    }
    ...
    <div className="container mx-auto p-4 max-w-6xl">
      ...
      <div className="grid grid-cols-1 gap-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                />
                {currentPersonal?.rol && (
                  <p className="text-xs text-muted-foreground mt-1">Rol: {currentPersonal.rol}</p>
                )}
              </div>
          ...
          <CardContent>
            <div className="border rounded-lg overflow-hidden">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="bg-sky-700 text-white">Nombre</TableHead>
                    <TableHead className="bg-sky-700 text-white">Pieza</TableHead>
                    <TableHead className="bg-sky-700 text-white text-center">Cantidad</TableHead>
                    <TableHead className="bg-sky-700 text-white text-right">P. Unitario</TableHead>
                    <TableHead className="bg-sky-700 text-white text-center">Descuento %</TableHead>
                    <TableHead className="bg-sky-700 text-white text-right">Total</TableHead>
                    <TableHead className="bg-sky-700 text-white text-center">Eliminar</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {items.map((item, index) => (
                    <TableRow key={index} className="hover:bg-muted">
                      <TableCell className="font-medium text-sm">{item.procedimiento_nombre}</TableCell>
                      <TableCell>
                        <Input
                          type="text"
                          autoComplete="off"
                          value={item.pieza_dental}
                          onChange={(e) => handleUpdateItem(index, 'pieza_dental', e.target.value)}
                          placeholder="Ej: 1.1"
                      ...
                      <TableCell>
                        <div className="text-center">
                          <Input
                            ...
                            onChange={(e) => handleUpdateItem(index, 'descuento_porcentaje', e.target.value)}
                            min="0"
                            max="100"
                            className="h-8 text-xs text-center"
                          />
            ...
            {items.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">
                No hay procedimientos. Haz clic en &quot;Agregar Procedimiento&quot; para comenzar.
              </div>
            )}
            ...
            {items.length > 0 && (
              <div className="mt-4 flex justify-end">
                <div className="text-right">
                  ...
                  <p className="text-2xl font-bold">{currencySymbol}{subtotal.toFixed(2)}</p>
                </div>
              </div>
            )}
          </CardContent>
        ...
        <div className="flex justify-end gap-2">
          <Button variant="outline" onClick={() => router.push(`/admin/ficha-odontologica/${numeroHistoria}/casos/${casoId}/presupuesto`)}>
            Cancelar
          </Button>
