...
export function PresupuestoForm({
  ...
  const [fecha, setFecha] = useState(
    presupuesto
      ? new Date(presupuesto.costo_total || 0).toISOString().split("T")[0]
  ...
  const [items, setItems] = useState<PlanItem[]>(
    presupuesto?.items_json?.map((item) => ({
      ...
      procedimiento_nombre:
        ...
        `Procedimiento ${item.procedimiento_id.slice(0, 8)}`,
  ...
  useEffect(() => {
    if (presupuesto?.items_json && Array.isArray(presupuesto.items_json)) {
      const loadedItems: PlanItem[] = presupuesto.items_json.map(
        (item: StoredPlanItem) => ({
          procedimiento_id: item.procedimiento_id,
  ...
  }, [presupuesto?.id, presupuesto?.items_json, presupuesto?.moneda_id]); // Dependencias completas
    ...
    const convertPrices = async (newMonedaId: string) => {
      ...
      try {
        ...
        (preciosData || []).forEach((p: PrecioData) => {
          if (p && p.procedimiento_id)
            priceMap[p.procedimiento_id] = Number(p.precio);
        });
