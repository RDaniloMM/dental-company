  ...
  const fetchPresupuestos = useCallback(async () => {
    ...
    try {
      ...
      const medicoIds = Array.from(
        new Set(presupuestosRaw.map((p) => p.medico_id).filter(Boolean))
      ) as string[];
      ...
      if (medicoIds.length > 0) {
        const { data: medicosData } = await supabase
          ...
          .in("id", medicoIds);
        if (medicosData) {
          medicoMap = (medicosData as MedicoData[]).reduce(
            (acc: Record<string, { nombre_completo: string }>, m) => {
      ...
      const monedaIds = Array.from(
        new Set(presupuestosRaw.map((p) => p.moneda_id).filter(Boolean))
      ) as string[];
      ...
      if (monedaIds.length > 0) {
        const { data: monedasData } = await supabase
          ...
          .in("id", monedaIds);
        if (monedasData) {
          monedaMap = (monedasData as MonedaData[]).reduce(
      ...
      const transformed = presupuestosRaw.map((item) => ({
        id: item.id,
        nombre: item.nombre,
  ...
  return (
    <div className='border rounded-lg overflow-hidden'>
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead className='bg-sky-700 text-white font-semibold'>
              Secuencia
            </TableHead>
            <TableHead className='bg-sky-700 text-white font-semibold'>
              Tratamiento
            ...
            <TableHead className='bg-sky-700 text-white font-semibold text-right'>
              Total
            </TableHead>
            <TableHead className='bg-sky-700 text-white font-semibold'>
              Pago
            </TableHead>
            <TableHead className='bg-sky-700 text-white text-right font-semibold'>
              Acciones
            </TableHead>
          </TableRow>
        </TableHeader>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Esta acción eliminará el presupuesto y todos sus
                              items. No se puede deshacer.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
