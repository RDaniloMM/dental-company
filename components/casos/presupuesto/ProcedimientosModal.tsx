...
interface ProcedimientosModalProps {
  ...
  monedaId: string;
  pacienteId?: string; // Se mantiene en la interfaz para compatibilidad pero no se usa actualmente
}
...
export function ProcedimientosModal({
  ...
  onSelectProcedimientos,
  monedaId,
}: // pacienteId se omite intencionalmente - reservado para uso futuro
      ...
      try {
        ...
        const formatted = (data || []).map(
          (p: {
            ...
            nombre: string;
            descripcion: string;
            procedimiento_precios: Array<{ precio: number }>;
          }) => ({
            id: p.id,
            nombre: p.nombre,
              ...
              <>
                {missingMoneda && (
                  <div className='mb-4 p-3 rounded bg-yellow-50 text-yellow-800 border border-yellow-100 text-sm'>
                    Selecciona una moneda en el formulario para ver los precios
                    disponibles.
                  </div>
                )}
