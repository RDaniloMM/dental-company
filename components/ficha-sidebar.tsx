  ...
  useEffect(() => {
    const handler = (e: Event) => {
      try {
        ...
        if (!updated) return;
        if (
          (updated.id && updated.id === patientId) ||
          (updated.numero_historia &&
            updated.numero_historia === patient?.numero_historia)
        ) {
          setPatient(updated);
    ...
    return () =>
      window.removeEventListener("paciente-updated", handler as EventListener);
  }, [patientId, patient]);
