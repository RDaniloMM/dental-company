  ...
  const handleViewReporte = async (paciente: Paciente) => {
    ...
    setReporteInfo({
      ...
      casosActivos: casosData.filter(
        (c) => c.estado === "Activo" || c.estado === "En Progreso"
      ).length,
        ...
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            <DialogTitle className='flex items-center gap-2'>
              ...
              Reportes de {selectedPaciente?.nombres}{" "}
            ...
            <div className='space-y-6'>
              ...
              {reporteInfo.ultimaCita && (
                <p className='text-sm text-muted-foreground'>
                  Última cita:{" "}
                  {new Date(reporteInfo.ultimaCita).toLocaleDateString(
                    "es-PE",
                    {
                      weekday: "long",
                      year: "numeric",
