...
export async function upsertPresupuesto(formData: unknown, items: unknown) {
  ...
  if (!parsedData.success) {
    return { error: { message: 'Datos del presupuesto inválidos.', issues: parsedData.error.issues } }
  }

  const parsedItems = z.array(planItemSchema).safeParse(items)
  if (!parsedItems.success) {
    return { error: { message: 'Datos de procedimientos inválidos.', issues: parsedItems.error.issues } }
  }
  ...
  try {
    ...
    if (isUpdate) {
      const { data: existing } = await supabase.from('presupuestos').select('creador_personal_id, creador_nombre, creador_rol').eq('id', presupuestoId).single()
      if (existing) {
        creador_personal_id = creador_personal_id || existing.creador_personal_id || null
    ...
    if (presupuestoError) {
      console.error('[presupuesto] Error upserting presupuestos:', presupuestoError)
...
export async function deletePresupuesto(id: string) {
  ...
  try {
    ...
    const { data: presupuesto, error: fetchError } = await supabase.from('presupuestos').select('creador_personal_id, creador_rol, creador_nombre').eq('id', id).single()
    if (fetchError) {
    ...
    if (user.email) {
      const { data: personalMatch } = await supabase
        ...
        .ilike('email', user.email)
        .limit(1)
      if (personalMatch && personalMatch.length > 0) currentPersonal = personalMatch[0]
    }

    const isAdmin = currentPersonal?.rol === 'Admin'
    const isCreator = currentPersonal?.id && presupuesto?.creador_personal_id && currentPersonal.id === presupuesto.creador_personal_id

    if (!isAdmin && !isCreator) {
      return { error: { message: 'No autorizado para eliminar este presupuesto.' } }
    }
    ...
    const errorMessage = error instanceof Error ? error.message : 'Ocurrió un error desconocido'
    return { error: { message: `Error al eliminar el presupuesto: ${errorMessage}` } }
  }
}
